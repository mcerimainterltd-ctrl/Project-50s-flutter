// XameDiscover — Unprecedented award-winning Discovery screen
// Architecture: Stories → Region filter → People → Trending feed
import 'dart:math';
...
// ── Mock data provider — swap for real API when server endpoints exist ────────
...
class XameDiscoverScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<XameDiscoverScreen> createState() =>
      _XameDiscoverScreenState();
}
...
class _XameDiscoverScreenState extends ConsumerState<XameDiscoverScreen>
  ...
  Future<void> _loadData() async {
    ...
    // Simulate network delay — replace with real API calls
          ...
          slivers: [
            ...
            SliverAppBar(
              ...
              actions: [
                ...
                IconButton(
                  icon: const Icon(
                    Icons.tune_rounded, color: Colors.white70),
            ...
            SliverToBoxAdapter(
              child: DiscoveryStoriesBar(
                users: List.generate(8, (i) => {
                  'name':     i == 0 ? 'You' : _people.isNotEmpty
                    ? _people[i % _people.length].name.split(' ')[0] : 'User $i',
                  'avatar':   _people.isNotEmpty
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    prefixIcon: const Icon(
                      Icons.search, color: Colors.white38, size: 20),
                    filled:     true,
                    ...
                    itemBuilder: (_, i) {
                      ...
                      return ListTile(
                        ...
                        trailing: item.isLive
                          ? const LivePulseIndicator(compact: true)
                          : null,
                      );
                    },
                  ),
...
class _FilterSheetState extends State<_FilterSheet> {
  ...
  void initState() {
    super.initState();
    _selected = widget.currentRegion;
  }
          ...
          child: Column(
            ...
            children: [
              Row(children: [
                ...
                const Spacer(),
                Text(
                  '${_fmtCount(item.viewCount)} views · '
                  '${_fmtCount(item.likeCount)} likes',
                  style: const TextStyle(
                    color: Colors.white38, fontSize: 12)),
              ]),
