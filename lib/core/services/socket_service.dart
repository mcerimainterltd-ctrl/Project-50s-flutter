...
class SocketService {
  ...
  void _scheduleReconnect(String xameId, {bool stealth = false}) {
    if (_reconnectAttempts >= AppConstants.maxReconnectAttempts) {
      _connectionStateCtrl.add(SocketState.failed); return;
    }
    final delay = min(AppConstants.reconnectBaseDelayMs * pow(1.5, _reconnectAttempts), 15000).toInt();
    _reconnectAttempts++;
  ...
  void _registerHandlers(IO.Socket socket, String xameId, {bool stealth = false}) {
    ...
    socket.on('reconnect', (_) {
      ...
      _connectionStateCtrl.add(SocketState.connected);
      if (!stealth) emit('user-online', {'userId': xameId, 'timestamp': DateTime.now().millisecondsSinceEpoch});
      emit('request_online_users', null);
    });
    ...
    socket.onDisconnect((_) {
      _offlineTimer?.cancel();
      _offlineTimer = Timer(Duration(milliseconds: AppConstants.offlineGracePeriodMs),
        () { if (!isConnected) _connectionStateCtrl.add(SocketState.disconnected); });
    });

    socket.on('receive-message', (d) { if (d != null) _receiveMessageCtrl.add(Map<String,dynamic>.from(d)); });
    socket.on('typing',          (d) { final s = d?['senderId'] as String?; if (s != null) _typingCtrl.add(s); });
    socket.on('stop-typing',     (d) { final s = d?['senderId'] as String?; if (s != null) _stopTypingCtrl.add(s); });

    socket.on('message-status-update', (d) { if (d != null) _msgStatusCtrl.add(MsgStatusUpdate(
      recipientId: d['recipientId'], messageId: d['messageId'], status: d['status'])); });

    socket.on('message-seen-update', (d) { if (d != null) _msgSeenCtrl.add(MsgSeenUpdate(
      recipientId: d['recipientId'], messageIds: List<String>.from(d['messageIds'] ?? []))); });

    socket.on('online_users',   (d) => _onlineUsersCtrl.add(List<String>.from(d ?? [])));
    socket.on('chat_history',   (d) => _chatHistoryCtrl.add(d));
    socket.on('contacts_list',  (d) {
      if (d == null || d is! List) return;
      _contactsListCtrl.add(List<Map<String,dynamic>>.from((d as List).map((c) => Map<String,dynamic>.from(c))));
    });

    socket.on('contact-status-update', (d) { if (d != null) _contactStatusCtrl.add(
      ContactStatusData(userId: d['userId'], status: '${d['status']?['emoji'] ?? ''} ${d['status']?['message'] ?? ''}'.trim())); });

    socket.on('profile-updated',      (d) { if (d != null) _profileUpdatedCtrl.add(Map<String,dynamic>.from(d)); });
    socket.on('new_missed_call_count', (d) { final s = d?['senderId'] as String?; if (s != null) _missedCallCountCtrl.add(s); });

    socket.on('messages-deleted', (d) { if (d != null) _messagesDeletedCtrl.add(MessagesDeletedData(
      deleterId: d['deleterId'], contactId: d['contactId'],
      messageIds: List<String>.from(d['messageIds'] ?? []), permanently: d['permanently'] ?? false)); });

    socket.on('disappearing:expired', (d) { if (d != null) _disappearExpiredCtrl.add(
      DisappearExpiredData(messageId: d['messageId'], contactId: d['contactId'])); });

    socket.on('call-user', (d) { if (d != null) _incomingCallCtrl.add(IncomingCallData(
      offer: d['offer'], callerId: d['callerId'],
      caller: Map<String,dynamic>.from(d['caller'] ?? {}),
      callType: d['callType'] ?? 'voice', callId: d['callId'])); });

    socket.on('make-answer',      (d) { if (d != null) _callAnswerCtrl.add(CallAnswerData(answer: d['answer'], senderId: d['senderId'])); });
    socket.on('ice-candidate',    (d) { if (d != null) _iceCandidateCtrl.add(IceCandidateData(candidate: d['candidate'], senderId: d['senderId'])); });
    socket.on('call-accepted',    (d) { if (d?['recipientId'] != null) _callAcceptedCtrl.add(d['recipientId']); });
    socket.on('call-rejected',    (d) => _callRejectedCtrl.add(CallRejectedData(senderId: d?['senderId'], reason: d?['reason'] ?? 'user-rejected')));
    socket.on('call-acknowledged',(d) { if (d?['senderId'] != null) _callAcknowledgedCtrl.add(d['senderId']); });
    socket.on('call-ended',       (d) => _callEndedCtrl.add(d?['senderId'] ?? ''));

    socket.on('wallet:receive', (d) { if (d != null) _walletReceiveCtrl.add(WalletReceiveData(
      senderId: d['senderId'], senderName: d['senderName'],
      amount: (d['amount'] as num).toDouble(), currency: d['currency'] ?? 'USD')); });

    socket.on('force-logout', (d) => _forceLogoutCtrl.add(d?['reason'] ?? 'You have been logged out remotely.'));
  }
  ...
  void emitCallUser(String r, dynamic offer, String t) => emit('call-user',          {'recipientId': r, 'offer': offer, 'callType': t});
  void emitMakeAnswer(String r, dynamic answer)      => emit('make-answer',          {'recipientId': r, 'answer': answer});
  ...
  void startHeartbeat(String xameId, {bool stealth = false}) {
    stopHeartbeat();
    _heartbeatTimer = Timer.periodic(Duration(milliseconds: AppConstants.heartbeatIntervalMs),
  ...
  void startStealthMode(String xameId) {
    ...
    _stealthTimer = Timer.periodic(Duration(milliseconds: AppConstants.stealthHeartbeatMs),
...
class IncomingCallData {
  final dynamic offer; final String callerId, callType; final String? callId; final Map<String,dynamic> caller;
...
class MessagesDeletedData {
  final String deleterId, contactId; final List<String> messageIds; final bool permanently;
