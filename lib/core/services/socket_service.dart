...
class SocketService {
  ...
  final _collabCancelledCtrl = StreamController<Map<String,dynamic>>.broadcast();
  ...
  Stream<Map<String,dynamic>>       get collabCancelled => _collabCancelledCtrl.stream;
  ...
  void _registerHandlers(IO.Socket socket, String xameId,
    ...
    socket.on('collab_cancelled', (d) {
      if (d != null) _collabCancelledCtrl.add(Map<String,dynamic>.from(d as Map));
    });
    socket.on('collab_authorized', (d) {
      if (d != null) _collabAuthorizedCtrl.add(Map<String,dynamic>.from(d as Map));
    });
    socket.on('collab_submitted', (d) {
      if (d != null) _collabSubmittedCtrl.add(Map<String,dynamic>.from(d as Map));
    });
